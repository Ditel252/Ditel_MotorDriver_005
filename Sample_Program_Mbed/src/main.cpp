...
int main(void){
    ...
    while(true){
        //正転でだんだん速くしていく
        for(int i = 0; i < 200; i++){
            motorDriverRotate(0b0001, MOTOR_FORWARD, i);
        ...
        //ニュートラルにする
        motorDriverRotate(0b0001, MOTOR_NEUTRAL);
        ThisThread::sleep_for(1000ms);


        //反転でだんだん速くしていく
        for(int i = 0; i < 200; i++){
            motorDriverRotate(0b0001, MOTOR_REVERSAL, i);
            ThisThread::sleep_for(200ms);
        }
        ...
        //ブレーキをかける
        motorDriverRotate(0b0001, MOTOR_BRAKE);
        ThisThread::sleep_for(1000ms);
    }
}